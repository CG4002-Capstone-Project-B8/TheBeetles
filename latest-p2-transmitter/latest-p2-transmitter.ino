...
uint8_t S_REPEATS = 1;
uint8_t PLAYER_TWO_SEND_COMMAND = 0xCC;

volatile bool has_sent_shot = false;
uint16_t PLAYER_TWO_ADDRESS = 0x2B12; // Data to be encoded in the IR beam
uint32_t PLAYER_TWO_RAW_DATA = 0xAA2B12;
...
void checkIfSentShot() {
  if (has_sent_shot) {
    ...
    IrSender.sendNEC(PLAYER_TWO_ADDRESS, PLAYER_TWO_SEND_COMMAND, S_REPEATS);
    // IrSender.sendNECRaw(PLAYER_TWO_RAW_DATA, SEND_REPEATS);
    digitalWrite(LED_PIN, HIGH);
    led_on_time = millis();
